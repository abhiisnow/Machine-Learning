# coding: utf-8
import Unification_Service_Machine_Learning_Application.Machine_Learning_Tool.SavewithAllValues as SavewithAllValues
import datetime
import logging
import os
import re
...
#why these methods? You cannot add predictions drectly to the training file
...
class Create_User_Interface:
    def launching_user_interface(self):
        print(colored('!!!Important Information!!!\nAll files will be saved at '
                      + os.getcwd() + '\\Data Sets\\'
                      + '\nPlease check the dirctory for the files created.', 'green'))

        parser = GooeyParser(description="LDA Unification Service!!!")


        parser.add_argument("input_file_name", help="Input Filename", widget="FileChooser")

        parser.add_argument("input_file_sheet_name", default="Enter Sheet Name",
                            help="Input Sheet Name(First sheet in case of multiple sheet processing.) if the file is a "
                                 "excel sheet (.xlsx)")

        parser.add_argument("column_name", default="Initial_Value",
                            help="Input Column Name(s) to process from input file")

        parser.add_argument("column_prediction_sorting_order", default="1",
                            help="Please confirm the order of sorting to save the file.\nPress 1. Best to Worst\n"
                                 "Press 2. Worst to Best \nYour Response:")

        # parser.add_argument('-mss', '--MultiSheet',
        #                     action="store_true", help="Check this box if There are multiple Sheets (default: "
        #                                               "Unification Service will start for 1 sheet)",
...
class Process_All_Text_Columns:
    def ml_process_general_text_columns_processing(self):
        """
        :return: na
        """
        ...
        """Calling the constructor of user interface to start the ui
        and then getting the user inputs"""
        create_ui = Create_User_Interface()
        args = create_ui.launching_user_interface()
        ...
        column_name_list = [x.strip() for x in args.column_name.split(',')]
            ...
            while (input_file_path == "0"):
                ...
                if (os.path.isfile(input_file_name)):
                    ...
                    while (input_file_name_exists == '0'):
                        ...
                        not supported"""
                        if input_file_name.rpartition('.')[2] == 'xlsx':
                            input_file_sheet_name = args.input_file_sheet_name
                            input_data = pd.read_excel(mapped_file, sheet_name=input_file_sheet_name,
                                                         encoding='utf-8', na_filter=False, header=0)
                            ...
                            if input_data.empty:
                                ...
                                input_data = pd.read_csv(mapped_file, encoding='utf-8', header=0, na_filter=False,
                    ...
                    for header in header_names:
                        ...
                        if (header == column_name or header_with_underscore == column_name):
                                    ...
                                    while (picklist_column_file_path == '0'):
                                        ...
                                        if (os.path.isfile(picklist_column_file)):
                                            ...
                                            while (picklist_column_file_exists == '0'):
                                                if (picklist_column_file.rpartition('.')[2] == 'csv'):
                                                   ...
                                                   # logging.info('Train File Created')
                                                    ...
                                                    picklist_name_file_exists = args.picklist_name_file_exists
                                        else:
                                            ...
                                            picklist_column_file_path_message_colored = colored(
                                                ...
                                                'blue')
                                            #logging.info('Train File not found')
                                            picklist_column_file_path_message = args.picklist_column_file_path_message
                            ...
                            while (column_prediction_sorting_order == '0'):
                                ...
                                worst or vice-versa"""
                                column_prediction_sorting_order = args.column_prediction_sorting_order
                                if (column_prediction_sorting_order == '1'):
                                    ...
                                    column_prediction_sorting_message_colored = colored(
                                        ...
                                        'blue')
                                    column_prediction_sorting_message = args.column_prediction_sorting_message
        ...
        print(colored('!!!Important Information!!!\nThe log file with the information shown here is saved at '
                      + os.getcwd() + '\\' + log_file_name
                      + '\nPlease check the file for details.', 'green'))


# main method to run the Text processing
@Gooey(dump_build_config=True, program_name="LDA Multi purpose Tool")
def main():
    ...
    object_process_all_text_columns.ml_process_general_text_columns_processing()
