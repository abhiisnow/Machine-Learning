...
data.loc[:,'file_data']=data.loc[:,'file_data'].apply(lambda x: x[0:3000])
...
model = Sequential()
model.add(Embedding(input_dim = vocab_size, output_dim=200, trainable=False, embeddings_initializer=Constant(embedding_matrix) ,input_length=300))
...
model.save(wd + '\\models\\model_file(first_3000c).h5') 

#%% use model
model = load_model(wd + '\\models\\model_file(first_3000c).h5') 
